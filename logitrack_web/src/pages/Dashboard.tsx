...
export function Dashboard() {
  ...
  useEffect(() => { if (!isSupervisor) branchApi.list("activo").then(setBranches); }, [isSupervisor]);

  const fetchDashboard = useCallback((showLoading: boolean) => {
    if (showLoading) setLoading(true); else setIsRefreshing(true);
    ...
    Promise.all([
      ...
      if (showLoading) setLoading(false); else setIsRefreshing(false);
    }).catch(() => {
      if (showLoading) setLoading(false); else setIsRefreshing(false);
    });
    ...
    (async () => {
      ...
      try {
        const results = await Promise.all(Array.from({length:7}, (_,i) => {
          ...
          if (effectiveBranch) p.branch_id = effectiveBranch;
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto" ref={dashboardRef}>
      <PageHeader title="Dashboard" description="Vista consolidada de la operaci\u00f3n log\u00edstica"
