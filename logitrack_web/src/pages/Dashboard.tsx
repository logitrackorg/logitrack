      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
            <label htmlFor="date-from" style={{ color: "#6b7280" }}>
              Desde
            </label>
            <input
              id="date-from"
            ...
            />
            <label htmlFor="date-to" style={{ color: "#6b7280" }}>
              Hasta
            </label>
            <input
              id="date-to"
  ...
  return (
    <div
          ...
          return (
            <g key={d.date}>
              ...
              <rect
                ...
                opacity={0.85}>
                <title>
                  {d.date} — Creados: {d.created}
                </title>
              </rect>
              {d.created > 0 && createdH > 14 && (
                <text
                  x={xCreated + barW / 2}
                  y={chartH - createdH + 8 + createdH - 4}
                  textAnchor="middle"
                  fontSize={9}
                  fill="white"
                  fontWeight={600}>
              ...
              <rect
                ...
                opacity={0.85}>
                <title>
                  {d.date} — Entregados: {d.delivered}
                </title>
              </rect>
              {d.delivered > 0 && deliveredH > 14 && (
                <text
                  x={xDelivered + barW / 2}
                  y={chartH - deliveredH + 8 + deliveredH - 4}
                  textAnchor="middle"
                  fontSize={9}
                  fill="white"
                  fontWeight={600}>
      ...
      <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 4, paddingLeft: 40, display: "flex", gap: 16 }}>
        <span>
          Creados en el período: <strong style={{ color: "#374151" }}>{days.reduce((s, d) => s + d.created, 0)}</strong>
        </span>
        <span>
          Entregados en el período:{" "}
          <strong style={{ color: "#374151" }}>{days.reduce((s, d) => s + d.delivered, 0)}</strong>
        </span>
      </div>
    </div>
  );
