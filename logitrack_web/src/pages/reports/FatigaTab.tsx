...
function riskBadge(level: string): { bg: string; text: string; label: string } {
  switch (level) {
    case "verde":    return { bg: "bg-emerald-100", text: "text-emerald-700", label: "Bajo" };
    case "amarillo": return { bg: "bg-amber-100",   text: "text-amber-700",   label: "Medio" };
    case "rojo":     return { bg: "bg-rose-100",    text: "text-rose-700",    label: "Alto" };
    default:         return { bg: "bg-slate-100",   text: "text-slate-500",   label: "Sin datos" };
  }
}
...
export function FatigaTab({ branchId }: FatigaTabProps) {
  ...
  const isEmpty = !data || data.daily_metrics.length === 0;

  const exportPDF = useCallback(async () => {
    await exportToPDF(contentRef, `fatiga_conductores_${new Date().toISOString().slice(0, 10)}.pdf`);
  }, []);
  ...
  const exportExcel = useCallback(() => {
    ...
    if (isEmpty) {
      exportToExcel(
        [{ name: "Fatiga", data: [{ Mensaje: "No hay datos de fatiga para el período seleccionado" }] }],
        `fatiga_conductores_${date}.xlsx`,
      );
      return;
    }
    const dailyRows = data!.daily_metrics.map((m) => ({
      Fecha:                       m.date,
      "Verde (bajo riesgo)":       m.verde,
      "Amarillo (riesgo medio)":   m.amarillo,
      "Rojo (alto riesgo)":        m.rojo,
      Salteado:                    m.salteado,
      "Horas sueño prom.":         m.avg_horas_sueno,
      "Score riesgo prom.":        m.avg_risk_score,
    }));
    const rankingRows = data!.driver_ranking.map((r, i) => ({
      Posición:              i + 1,
      Conductor:             r.full_name,
      Usuario:               r.username,
      "Score riesgo prom.":  r.avg_risk_score,
      "Nivel riesgo":        r.risk_level,
      "Horas sueño prom.":   r.avg_horas_sueno > 0 ? r.avg_horas_sueno : "Sin datos",
      "Total check-ins":     r.total_checkins,
    }));
    exportToExcel(
      [
        { name: "Métricas diarias",    data: dailyRows },
        { name: "Ranking conductores", data: rankingRows },
      ],
      `fatiga_conductores_${date}.xlsx`,
    );
  }, [data, isEmpty]);
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-900 dark:text-gray-100">
            Métricas de Fatiga de Conductores
          </h2>
        </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/50">
                        <th className="text-left px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">#</th>
                        <th className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Conductor</th>
                        <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Riesgo prom.</th>
                        <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Nivel</th>
                        <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Sueño prom.</th>
                        <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Check-ins</th>
                      </tr>
                    </thead>
                    <tbody>
