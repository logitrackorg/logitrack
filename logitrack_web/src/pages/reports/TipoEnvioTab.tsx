...
import { reportsApi, type ShipmentTypeBucket } from "../../api/reports";
...
export default function TipoEnvioTab({ dateFrom, dateTo, branchId }: TipoEnvioTabProps) {
  ...
  const [buckets, setBuckets] = useState<ShipmentTypeBucket[]>([]);
  ...
  const chartData = TYPE_ORDER.map((t) => {
    const bucket = buckets.find((b) => b.shipment_type === t);
  ...
  const exportPDF = useCallback(async () => {
    await exportToPDF(contentRef, `tipo_envio_${new Date().toISOString().slice(0, 10)}.pdf`);
  }, []);

  const exportExcel = useCallback(() => {
    const rows = chartData.map((d) => ({
      "Tipo de envío": d.label,
      Cantidad: d.count,
      Porcentaje: total > 0 ? `${d.pct.toFixed(1)}%` : "0%",
    }));
    exportToExcel(
      [{ name: "Distribución por tipo", data: rows }],
      `tipo_envio_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  }, [chartData, total]);
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-900">Distribución por tipo de envío</h2>
        </div>
