...
export function ReportIncidents() {
  ...
  const [selectedBranch, setSelectedBranch] = useState(isSupervisor ? supervisorBranch : "");
  ...
  useEffect(() => {
    if (isSupervisor && !supervisorBranch) return;
    setLoading(true);
    setError(false);
    ...
    <div className="p-6 max-w-[1400px] mx-auto" ref={reportRef}>
      <Breadcrumb
        items={[
          ...
          { label: "Reclamos por Sucursal" },
        ]}
      />

      <PageHeader
        title="Reclamos por Sucursal"
        description="Incidentes reportados por sucursal con desglose por tipo"
