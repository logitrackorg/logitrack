...
func (h *CoverageHandler) Diagnose(c *gin.Context) {
	...
	if len(req.ExcludedBranchIDs) == 0 {
		c.JSON(http.StatusOK, h.svc.Diagnose(req.AreaKm2, req.CustomBoundingArea, req.DangerousZones, req.IncludeInactive, req.MaxSuggestions, req.SnapToCities, req.Mode, req.MinPopulation))
	} else {
		c.JSON(http.StatusOK, h.svc.DiagnoseExcluding(req.AreaKm2, req.ExcludedBranchIDs, req.CustomBoundingArea, req.DangerousZones, req.IncludeInactive, req.MaxSuggestions, req.SnapToCities, req.Mode, req.MinPopulation))
	}
}
