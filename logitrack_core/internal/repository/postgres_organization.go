...
func (r *postgresOrganizationRepository) Get() (*model.OrganizationConfig, error) {
	...
	err := row.Scan(&cfg.ID, &cfg.Name, &cfg.CUIT, &cfg.Address, &cfg.Phone, &cfg.Email, &cfg.TrackURL,
		&cfg.PrimaryColor, &cfg.AccentColor, &cfg.SidebarColor, &cfg.LogoURL,
...
func (r *postgresOrganizationRepository) Upsert(config model.OrganizationConfig) (*model.OrganizationConfig, error) {
	row := r.db.QueryRow(`
		INSERT INTO organization_config (id, name, cuit, address, phone, email, track_url, primary_color, accent_color, sidebar_color, logo_url, updated_at, updated_by)
		...
		RETURNING id, name, cuit, address, phone, email, track_url, primary_color, accent_color, sidebar_color, logo_url, updated_at, updated_by
	`, config.Name, config.CUIT, config.Address, config.Phone, config.Email, config.TrackURL,
		config.PrimaryColor, config.AccentColor, config.SidebarColor, config.LogoURL, config.UpdatedBy)
