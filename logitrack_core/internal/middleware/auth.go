...
// AuthWithQueryParam is like Auth but also accepts the token via the ?token=
// query parameter. Intended for SSE / WebSocket endpoints where the browser's
// EventSource API cannot send custom request headers.
func AuthWithQueryParam(authRepo repository.AuthRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		header := c.GetHeader("Authorization")
		if !strings.HasPrefix(header, "Bearer ") {
