...
- Any intermediate state (`in_progress`, `pre_transit`, `in_transit`, `at_branch`, `delivering`, `delivery_failed`, `ready_for_pickup`, `ready_for_return`) can be cancelled via `CancelShipment` — `pending` and terminal states cannot.
...
- Supervisor + Manager + Admin: GET /stats, /vehicles/available, /vehicles/by-plate/:plate
- Supervisor + Admin: GET /users/drivers; PATCH /vehicles/by-plate/:plate/status; POST /vehicles/by-plate/:plate/assign, /assign-branch, /start-trip, /end-trip; DELETE /vehicles/by-plate/:plate/shipments/:trackingId
...
- Vehicle assignment panel in `ShipmentDetail`: supervisor + admin, shown when shipment is `in_progress`, `at_branch`, or `ready_for_pickup`
...
| GET | /api/v1/vehicles/available | Bearer | supervisor, manager, admin | List available vehicles (query: `type`, `min_capacity`, `branch_id`) |
...
| POST | /api/v1/vehicles/by-plate/:plate/assign | Bearer | supervisor, admin | Assign shipment to vehicle; transitions shipment to pre_transit |
