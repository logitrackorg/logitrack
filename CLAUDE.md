...
- **Operators**: `GET /shipments` is server-side filtered to their branch; they can only modify shipments in their branch (enforced in handler via `operatorBranchForbidden`). When creating shipments, `receiving_branch_id` is forced to their branch.
- **Supervisors**: can see all shipments but default to their branch in the list UI; can modify shipments in any branch. When creating, `receiving_branch_id` is forced to their branch.
- **Drivers**: `GET /users/drivers?branch_id=X` matches their `branch_id`; used when assigning drivers to delivering status.
- Manager and admin have no branch assignment.
...
- `✏️ Edit data` button in `ShipmentDetail`: operator + supervisor + admin; hidden on terminal statuses; hidden for operators viewing another branch's shipment
- `Cancel shipment` button in `ShipmentDetail`: only supervisor + admin; hidden for operators viewing another branch's shipment
...
- `VehicleList` branch filter: same pattern — operator locked, supervisor defaults to own branch
- `NewShipment` receiving branch: pre-filled and locked for operator + supervisor; free select for admin
- `ShipmentDetail` action buttons (Edit, Cancel, Update Status, Comments): hidden via `operatorOutOfBranch` for operators viewing shipments outside their branch
